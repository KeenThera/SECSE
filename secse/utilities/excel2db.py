...
def test_rxn(sma):
    ...
    try:
        ...
        print(f"Error processing SMARTS: {sma}\n{e}")
...
def main(excel_filename, output_type):
    ...
    for sheet_name in xls.sheet_names:
        if re.match(pattern, sheet_name):
            ...
            print(f"Finished processing {output_type} for {sheet_name}")
    ...
    if output_type == 'json':
        ...
        print("All sheets processed and JSON file created.")
...
if __name__ == "__main__":
    ...
    if output_type not in ['db', 'json']:
        print("Invalid output type. Please choose 'db' for database or 'json' for JSON file.")
    else:
